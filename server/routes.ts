...
router.post("/auth/oauth", async (req, res) => {
  try {
    ...
    if (!decoded.email) {
      return res
        .status(400)
        .json({ error: "Provider did not return an email" });
    }
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        description: `User setup account via token`,
      },
    });
  ...
  try {
    ...
    if (programManagerId) {
      coaches = await prisma.user.findMany({
        where: {
          role: "COACH",
          managerId: String(programManagerId),
          accountStatus: "ACTIVE",
          isActive: true,
        },
        select: { id: true, name: true },
    ...
    if (pathwayId) {
      const rawClasses = await prisma.classModel.findMany({
        where: { pathwayId: String(pathwayId), isActive: true },
  ...
  async (req, res) => {
    try {
      ...
      if (!cycle) {
        return res
          .status(400)
          .json({ error: "No open report cycle is available." });
      }
      ...
      const reportData = {
        ...
        challengesTags:
          typeof payload.challengesTags === "string"
      ...
      if (payload.status === "SUBMITTED") {
        ...
        if (payload.energy < thresholdEnergy)
          await createAlertIfNew(
            "LOW_ENERGY",
            "MEDIUM",
            "Student reported low energy",
          );
        if (payload.mood < thresholdMood)
          await createAlertIfNew(
            "LOW_MOOD",
            "MEDIUM",
            "Student reported low mood",
          );
        if (payload.attendance < thresholdAttend)
          await createAlertIfNew(
            "LOW_ATTENDANCE",
            "HIGH",
            "Attendance dropped below threshold",
          );
        if (payload.confidence < thresholdConf)
          await createAlertIfNew(
            "LOW_CONFIDENCE",
            "MEDIUM",
        ...
        if (payload.needsSupport)
          await createAlertIfNew(
            "SUPPORT_NEEDED",
            "HIGH",
            "Student explicitly requested support",
          );

        const hasBeginning = payload.classRatings?.some(
          (cr: any) => cr.rating === "BEGINNING",
        );
        if (hasBeginning)
          await createAlertIfNew(
            "LOW_PERFORMANCE",
            "HIGH",
            "Student reported BEGINNING in a class",
          );

        let parsedTags = [];
        try {
          parsedTags = JSON.parse(payload.challengesTags || "[]");
        } catch (e) {}
        if (parsedTags.length > 2)
          await createAlertIfNew(
            "CHALLENGE_FLAGGED",
            "MEDIUM",
            "Multiple challenges flagged",
          );
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
        ...
        include: {
          student: {
            include: {
              studentProfile: {
                include: {
                  classEnrollments: { include: { classModel: true } },
                },
              },
            },
          },
      ...
      else if (reqUser.role === "STUDENT" && report.studentId === reqUser.id)
        authorized = true;
      else if (
        reqUser.role === "COACH" &&
        report.student.studentProfile?.coachId === reqUser.id
      )
        authorized = true;
      else if (
        reqUser.role === "PROGRAM_MANAGER" &&
        report.student.studentProfile?.programManagerId === reqUser.id
      )
        authorized = true;
      else if (reqUser.role === "INSTRUCTOR") {
        const enrollments =
          report.student.studentProfile?.classEnrollments || [];
        authorized = enrollments.some(
          (ce) => ce.classModel?.instructorId === reqUser.id,
        );
      }
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          entityType: "USER",
          entityId: user.id,
          description: `Admin invited Program Manager ${name}`,
        },
      });
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      let cycle = await prisma.reportCycle.findFirst({
        where: { status: "OPEN" },
        orderBy: { createdAt: "desc" },
      });
      ...
      if (cycle) {
        ...
        const reviewedCount = await prisma.weeklyReport.count({
          where: { cycleId: cycle.id, status: "REVIEWED" },
        });
        if (totalStudents > 0)
          submissionRate = Math.round((submittedCount / totalStudents) * 100);
        if (submittedCount > 0)
          reviewedRate = Math.round((reviewedCount / submittedCount) * 100);
      }

      const openCyclesPastDue = await prisma.reportCycle.count({
      ...
      studentsNeedingSupport = await (prisma as any)
        .$queryRaw`SELECT COUNT(DISTINCT "studentId") as cnt FROM "WeeklyReport" WHERE "needsSupport" = 1`;
      if (
        typeof studentsNeedingSupport === "object" &&
        (studentsNeedingSupport as any)?.[0]?.cnt
      ) {
      ...
      const typeGroups = await prisma.alert.groupBy({
        by: ["type"],
        _count: { id: true },
      });
      const alertDistribution = typeGroups.map((g) => ({
        type: g.type,
        count: g._count.id,
      }));
      ...
      res.json({
        totalStudents,
        totalActiveUsers,
        totalProgramManagers: await prisma.user.count({
          where: { role: "PROGRAM_MANAGER", isActive: true },
        }),
        totalPathways: await prisma.pathway.count({
          where: { isActive: true },
        }),
        totalClasses: await prisma.classModel.count({
          where: { isActive: true },
        }),
        submittedReportsThisCycle: cycle
          ? await prisma.weeklyReport.count({ where: { cycleId: cycle.id } })
          : 0,
        submissionRate,
        reviewedRate,
        overdueReports,
        ...
        classPerformance: {
          overall: { EXCEEDING: 0, MEETING: 0, APPROACHING: 0, BEGINNING: 0 },
          byPathway: [],
        },
        submissionTrend: [],
        topCoaches: [],
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          entityType: "Pathway",
          entityId: pathway.id,
          description: `Created pathway ${name}`,
        },
      });
      res.json(pathway);
    } catch (e) {
    ...
    try {
      ...
      const updated = await prisma.appSettings.upsert({
        ...
        create: {
          ...
          weeklyDueDay:
            data.weeklyDueDay !== undefined
              ? parseInt(data.weeklyDueDay)
              : undefined,
          weeklyDueHour:
            data.weeklyDueHour !== undefined
              ? parseInt(data.weeklyDueHour)
              : undefined,
          autoCloseCycles: data.autoCloseCycles,
          alertThresholdEnergy:
            data.alertThresholdEnergy !== undefined
              ? parseInt(data.alertThresholdEnergy)
              : undefined,
          alertThresholdMood:
            data.alertThresholdMood !== undefined
              ? parseInt(data.alertThresholdMood)
              : undefined,
          alertThresholdAttend:
            data.alertThresholdAttend !== undefined
              ? parseInt(data.alertThresholdAttend)
              : undefined,
          alertThresholdConf:
            data.alertThresholdConf !== undefined
              ? parseInt(data.alertThresholdConf)
              : undefined,
  ...
  async (req: any, res: any) => {
    try {
      ...
      res.json(
        communities.map((c) => ({
          ...c,
          studentsCount: c._count.studentProfiles,
        })),
      );
    } catch (e) {
      res.status(500).json({ error: "Server error" });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          entityId: comm.id,
          description: `Created community: ${comm.name}`,
        },
      });
      res.json(comm);
    } catch (e) {
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      res.json(
        cycles.map((c) => ({ ...c, reportsCount: c._count.weeklyReports })),
      );
    } catch (e) {
      res.status(500).json({ error: "Server error" });
    }
    ...
    try {
      ...
      if (status === "OPEN") {
        ...
        if (hasOpen)
          return res
            .status(400)
            .json({ error: "An OPEN cycle already exists for this scope" });
      }
      ...
      await prisma.auditLog.create({
        data: {
          ...
          actorRole: (req as any).user?.role,
          action: "CREATE",
          entityType: "ReportCycle",
          entityId: cycle.id,
          description: `Created cycle: ${cycle.name}`,
        },
      });
      res.json(cycle);
    } catch (e) {
  ...
  async (req: any, res: any) => {
    try {
      ...
      if (status === "OPEN") {
        ...
        if (hasOpen && hasOpen.id !== id)
          return res
            .status(400)
            .json({ error: "An OPEN cycle already exists" });
      }
...
router.get(
  "/pm/dashboard",
  authMiddleware,
  roleMiddleware(["PROGRAM_MANAGER"]),
  async (req, res) => {
    try {
...
router.patch(
  "/reports/:id/review",
  authMiddleware,
  roleMiddleware(["ADMIN", "PROGRAM_MANAGER", "COACH"]),
  async (req, res) => {
    try {
      const { id } = req.params;
  ...
  async (req, res) => {
    try {
      const students = await prisma.user.findMany({
        where: {
          role: "STUDENT",
          studentProfile: { coachId: (req as any).user?.id },
        },
      });
      const reports = await prisma.weeklyReport.findMany({
        where: {
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      if (openCycle) {
        currentReport = await prisma.weeklyReport.findFirst({
          where: { studentId: req.user.id, cycleId: openCycle.id },
        });
      }
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      res.json(
        user?.studentProfile?.classEnrollments.map((e: any) => e.classModel) ||
          [],
      );
    } catch (e) {
      res.status(500).json({ error: "Server error" });
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      const myClasses = await prisma.classModel.findMany({
      ...
      const ratings = await prisma.classRating.findMany({
        ...
        include: {
          report: { include: { student: true, cycle: true } },
          classModel: true,
        },
      });
  ...
  async (req: any, res: any) => {
    try {
      const students = await prisma.user.findMany({
        ...
        include: {
          studentProfile: { include: { pathway: true, coach: true } },
        },
      });
      res.json(students);
    } catch (e) {
  ...
  async (req: any, res: any) => {
    try {
      const staff = await prisma.user.findMany({
        where: { role: { in: ["COACH", "INSTRUCTOR"] }, isActive: true },
      });
      res.json(staff);
    } catch (e) {
      res.status(500).json({ error: "Server error" });
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          entityType: "User",
          entityId: user.id,
          description: `PM invited ${role} ${name}`,
        },
      });
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      res.json(
        await prisma.community.findMany({ include: { programManager: true } }),
      );
    } catch (e) {
      res.status(500).json({ error: "Server error" });
    }
    ...
    try {
      res.json(
        await prisma.community.create({
          data: { name: req.body.name, programManagerId: req.user.id },
        }),
      );
    } catch (e) {
      res.status(500).json({ error: "Server error" });
    ...
    try {
      ...
      const totalStudents = await prisma.user.count({
        where: {
          role: "STUDENT",
          isActive: true,
          studentProfile: { programManagerId: pmId },
        },
      });
      ...
      if (cycle) {
        const submittedCount = await prisma.weeklyReport.count({
          where: {
            cycleId: cycle.id,
            student: { studentProfile: { programManagerId: pmId } },
          },
        });
        if (totalStudents > 0)
          submissionRate = Math.round((submittedCount / totalStudents) * 100);
