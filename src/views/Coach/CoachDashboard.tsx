...
export function CoachDashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <p className="text-xs text-[#6B7280] uppercase tracking-widest font-bold">Reports to Review</p>
        ...
        <Card className="p-6">
           <div className="flex items-center gap-4">
              ...
              <div>
                 ...
                 <p className="text-2xl font-black">{data.reports.filter((r:any) => r.needsSupport).length}</p>
              </div>
           </div>
        </Card>
      </div>
      ...
      <Card className="p-6">
         <h2 className="text-xl font-bold border-b pb-4 mb-4">Pending Student Reports</h2>
            ...
            {data.reports.map((r: any) => (
              <div key={r.id} className="p-5 border border-[#E5E7EB] rounded-xl bg-white flex justify-between items-center transition-all hover:border-[#FF7A00]">
                 <div>
                    ...
                    <p className="text-xs text-[#6B7280] mt-1">Topic: {r.weeklyTopic}</p>
                 </div>
                 <div className="flex gap-2">
                    <Button variant="outline" onClick={() => window.location.href = `/api/reports/export-pdf?id=${r.id}`}>PDF</Button>
                 </div>
              </div>
            ))}
            {data.reports.length === 0 && <p className="text-sm text-[#6B7280]">No pending reports.</p>}
         </div>
      </Card>
    </div>
  );
